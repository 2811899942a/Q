...
Motivation
----------
M12 established that DSSAT-native CLOUDS preserves almost all of the statistical
value of the external Kt gate, but direct additive shoulder cooling can produce
non-monotonic and physically impossible curves. M13 keeps the mechanism but
changes the mathematical form.

Rules
-----
- DTRc = 14.8 C, fixed from calibration-only breakpoint analysis.
- DTR <= DTRc: exact official HTEMP.
- Only two daytime subsegments are changed:
    1) solar noon -> modeled Tmax (pre-peak hot shoulder)
    2) modeled Tmax -> sunset (post-peak hot shoulder)
- Segment endpoint temperatures are exact anchors.
- The normalized official temperature fraction q in each segment is mapped by
      q_new = q ** p
  where
      p = 1 + k * (DTR-DTRc) * CLOUDS,  k >= 0.
- Because p>=1 and q is monotonic on each official segment, the transformed
  curve remains monotonic and cannot overshoot its segment endpoint temperatures.
...
def dssat_solar(date,srad):
 doy=date.timetuple().tm_yday;dec=-23.45*math.cos(2*PI*(doy+10.)/365.);soc=math.tan(RAD*dec)*math.tan(RAD*LAT);soc=min(max(soc,-1.),1.);dl=min(max(12.+24.*math.asin(soc)/PI,0.),24.);su=12.-dl/2.;sd=12.+dl/2.
 ss=math.sin(RAD*dec)*math.sin(RAD*LAT);cc=math.cos(RAD*dec)*math.cos(RAD*LAT);z=ss/cc if abs(cc)>1e-12 else 0.;z=min(max(z,-1.),1.);dsinb=3600.*(dl*ss+24./PI*cc*math.sqrt(max(0.,1.-z*z)));sclear=AMTRCS*S0N*dsinb*1e-6;cl=min(max(1.-srad/sclear,0.),1.) if sclear>0 else 0.
 return dl,su,sd,cl
def pl(h,tmax,tmin,dl,su,sd):
 mn=su+C;mx=mn+dl/2.+A;t=.5*PI*(sd-mn)/(mx-mn);ts=tmin+(tmax-tmin)*math.sin(t);tmini=(tmin-ts*math.exp(-B))/(1.-math.exp(-B));hdec=24.+C-dl
 if mn<=h<=sd:
  t=.5*PI*(h-mn)/(mx-mn);return tmin+(tmax-tmin)*math.sin(t)
 tt=24.+h-sd if h<mn else h-sd;return tmini+(ts-tmini)*math.exp(-B*tt/hdec)
def branch_info(r):
 h=float(r['solar_hour']);dl=r['dayl'];su=r['snup'];sd=r['sndn'];mx=su+C+dl/2.+A;tmax=float(r['tmax_ghcn_c']);tmin=float(r['tmin_ghcn_c']);p0=float(r['pred_c'])
 if 12.<h<mx:
  lo=pl(12.,tmax,tmin,dl,su,sd);hi=tmax;den=hi-lo
  if den<=1e-9:return 'none',0.,lo,hi
  q=min(max((p0-lo)/den,0.),1.);return 'pre',q,lo,hi
 if mx<h<sd:
  lo=pl(sd,tmax,tmin,dl,su,sd);hi=tmax;den=hi-lo
  if den<=1e-9:return 'none',0.,lo,hi
  q=min(max((p0-lo)/den,0.),1.);return 'post',q,lo,hi
 return 'none',0.,0.,0.
...
def fit_branch(rows,branch):
 # broad grid; if the best lands at the boundary, the script reports it explicitly.
 best=None;grid=[]
 for i in range(2001):
  k=i*.005  # 0 .. 10 per C DTR excess per unit CLOUDS
  sse=0.;n=0
  for r in rows:
   br,_,_,_=branch_info(r)
   if br!=branch or float(r['formal_dtr_c'])<=DTRC:continue
   p=transformed(r,k,0.) if branch=='pre' else transformed(r,0.,k)
   e=p-float(r['obs_c']);sse+=e*e;n+=1
  if n:
...
def main():
 ...
 bp,gp=fit_branch(cal,'pre');bq,gq=fit_branch(cal,'post');kpre=bp[1];kpost=bq[1]
 grid=[{'branch':'pre','k':k,'n':n,'cal_rmse':rm} for k,n,rm in gp]+[{'branch':'post','k':k,'n':n,'cal_rmse':rm} for k,n,rm in gq];write(GRID,grid,['branch','k','n','cal_rmse'])
 at_bound_pre=abs(kpre-10.)<1e-12;at_bound_post=abs(kpost-10.)<1e-12
 ...
 models=[('M0_OFFICIAL',lambda r:float(r['pred_c'])),('M13_MONOTONIC_CLOUD_WARP',lambda r:transformed(r,kpre,kpost))];groups=[('May-Sep',val),('DTR<14.8',[r for r in val if float(r['formal_dtr_c'])<DTRC]),('DTR>=15',[r for r in val if float(r['formal_dtr_c'])>=15]),('DTR>=18',[r for r in val if float(r['formal_dtr_c'])>=18])]
 rec=[]
 ...
 caldays={r['solar_date']:r['clouds'] for r in cal if float(r['formal_dtr_c'])>=15};vv=sorted(caldays.values());q1=vv[len(vv)//3];q2=vv[2*len(vv)//3];cr=[]
 ...
 # Complete-curve invariants. By construction these should all pass.
 daymeta={}
 ...
 for ds,r in daymeta.items():
  ...
  vals=[]
  for i in range(481):
   h=i*.05;p0=pl(h,tx,tn,dl,su,sd);fake=dict(r);fake['solar_hour']=str(h);fake['pred_c']=str(p0);pn=transformed(fake,kpre,kpost);vals.append((h,pn))
  rise=[z for z in vals if su+C<=z[0]<=mx];fall=[z for z in vals if mx<=z[0]<=sd];rd=[rise[i+1][1]-rise[i][1] for i in range(len(rise)-1)];fd=[fall[i+1][1]-fall[i][1] for i in range(len(fall)-1)]
 ...
 valchk=[r for r in checks if r['year']>=2017];bad=sum(r['rise_monotonic']=='NO' or r['fall_monotonic']=='NO' or r['below_tmin_c']>1e-6 or r['above_tmax_c']>1e-6 for r in valchk)
 mm={(r['model'],r['group']):r for r in rec};o=mm[('M0_OFFICIAL','DTR>=15')];n=mm[('M13_MONOTONIC_CLOUD_WARP','DTR>=15')];oa=mm[('M0_OFFICIAL','May-Sep')];na=mm[('M13_MONOTONIC_CLOUD_WARP','May-Sep')];imp=100*(o['rmse']-n['rmse'])/o['rmse'];impa=100*(oa['rmse']-na['rmse'])/oa['rmse']
