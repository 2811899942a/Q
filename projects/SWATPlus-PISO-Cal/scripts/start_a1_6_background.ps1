...
$Runtime = Join-Path $Root "artifacts\a1_6\runtime"
...
if (Test-Path -LiteralPath $PidPath) {
    ...
    if ($Existing -and (Get-Process -Id $Existing -ErrorAction SilentlyContinue)) { throw "A1.6 is already running with PID=$Existing" }
}
$Python = Join-Path $Root ".venv\Scripts\python.exe"
if (!(Test-Path -LiteralPath $Python)) { throw "project virtualenv Python not found: $Python" }
$Stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$Stdout = Join-Path $Runtime "a1_6_$Stamp.stdout.log"
$Stderr = Join-Path $Runtime "a1_6_$Stamp.stderr.log"
$Args = @("-u", "scripts\a1_6_local_enrichment.py", "--resume")
...
Write-Host "A1.6_BACKGROUND_STARTED"
Write-Host "PID=$($P.Id)"
