...
$Runtime = Join-Path $Root "artifacts\a2\runtime"
...
if (Test-Path -LiteralPath $PidPath) {
    ...
    if ($Existing -and (Get-Process -Id $Existing -ErrorAction SilentlyContinue)) { throw "A2 is already running with PID=$Existing" }
}
$Python = Join-Path $Root ".venv\Scripts\python.exe"
if (!(Test-Path -LiteralPath $Python)) { throw "project virtualenv Python not found: $Python" }
$Stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$Stdout = Join-Path $Runtime "a2_$Stamp.stdout.log"
$Stderr = Join-Path $Runtime "a2_$Stamp.stderr.log"
$Args = @("-u", "scripts\a2_ai_guided_region_test.py", "--resume")
...
Write-Host "A2_BACKGROUND_STARTED"
Write-Host "PID=$($P.Id)"
